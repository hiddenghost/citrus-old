<?php
namespace app\common\library\emchat;
...
//------------------------------------------------------用户体系	
	...
	*/
	...
	/*
	 *  target_type  users 单聊 chatgroups 群聊
	 * 发送文本消息 新方法
	*/
	function sendText2($from="admin",$target_type,$target,$content,$ext){
		$url=$this->url.'messages/' . $target_type;
		...
		$body['ext']=$ext;
		$header=array($this->getToken());
		$result=$this->postCurl($url, json_encode($body),$header);
		return $result;
	}
...
//		var_dump($filePath);
	...
	*/
	...
	/*
		创建推送标签
	*/
	function addPushLabel($name, $description){
		$url=$this->url.'push/label';
		$options=array(
			"name"=> $name,
	...
	/*
		查询指定的推送标签
	*/
	function getPushLabel($name){
		$url=$this->url.'push/label/'.$name;
		$header=array($this->getToken());
		$result=$this->postCurl($url,'',$header,'GET');
		return $result;
	}
	...
	*/
	 ...
	 * $usernames = ["hx1","hx2"];
	*/
	function addPushLabelUser($name, $usernames){
		$url=$this->url.'push/label/' . $name . '/user';
		$options=array(
			"usernames"=> $usernames,
		);
		$body=json_encode($options);
		log_write('在推送标签下添加用户-' . $body);
		$header=array($this->getToken(),'Content-Type:application/json');
		$result=$this->postCurl($url,$body,$header);
		return $result;
	}
	...
	/*
		分页查询指定标签下的用户
	*/
	function getPushLabelUsers($name, $start = 0){
		$url=$this->url.'push/label/' . $name . '/user?limit=100&cursor=' . $start;
		$header=array($this->getToken());
		$result=$this->postCurl($url,'',$header,'GET');
		return $result;
	}
	...
	/*
		批量移出指定推送标签下的用户
	*/
	function deletePushLabelUsers($name, $usernames){
		$url=$this->url.'push/label/'.$name . '/user';
		$options=array(
			"usernames"=> $usernames,
		);
	...
	/*
		以同步方式发送推送通知
	*/
	function pushsync($name, $message){
		$url=$this->url.'push/sync/' . $name;
		$options=array(
			"strategy" => 3,
			"pushMessage"=> $message,
		);
	...
	*/
	function pushsingle($usernames, $message){
	...
	/*
		...
		$usernames 为标签列表
	*/
	function pushlist($usernames, $message){
		$url=$this->url.'push/list/label';
		$options=array(
			"targets" => $usernames,
			"strategy" => 3,
			"pushMessage"=> $message,
