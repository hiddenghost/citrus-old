...
class Checkout
{
    ...
    private function setOrderExpress()
    {
        ...
        $list = $addressmodel->getList($this->user['user_id']);
        if(empty($list->toArray())){
            $this->setError("请添加收货地址");
        }
        $cityId = $this->user['address_default'] ? $this->user['address_default']['city_id'] : null;
        if ($cityId == 0) $this->setError("很抱歉，不支持您的收货城市");
        // 初始化配送服务类
        ...
        // 验证商品是否在配送范围
        $this->orderData['intra_region'] = ($notInRuleGoods === false);
