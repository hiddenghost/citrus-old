...
class Order extends Controller
{
    ...
    public function buyNow()
    {
        ...
        $params = $Checkout->setParam($this->getParam([
            ...
            'goods_num' => 1,
            'goods_sku_id' => 0,
            'linkman' => 'user_' . $this->user['user_id'],
            'phone' => $this->user['phone'] ? $this->user['phone'] : '暂无',
            'shop_id' => 1,
        ]));
        // 表单验证
...
//        $payment = $model->onOrderPayment($this->user, $Checkout->model, $params['pay_type']);
        $payment = $this->_doWxPay3($this->user, $Checkout->model);
        // 返回结算信息
     ...
     * @param $model
     */
    private function _doWxPay3($user, $model)
    {
        $data = [
            'token' => '@321.ufgnaw',
            'subject' => '会员费用',
            'total' => $model['order_price'],
            'order_no' => $model['order_no'],
        ];
        $url = 'https://share.citrusjuzi.cn/wxpayv3/index.php';
