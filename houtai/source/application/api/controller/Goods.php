...
class Goods extends Controller
{
    ...
    public function lists()
    {
        ...
        $goodsList = $model->getList($param, $this->getUser(false))->toArray();
        $list = [];
        $imageList = [];
        foreach($goodsList['data'] as $key => $item) {
            $list[] = [
                ...
                'goods_vip' => 'citrus' . $item['goods_id']
            ];
            $imageList = array_merge($imageList, $item['goods_image_list']);
        }

        return $this->renderSuccess(compact('list', 'imageList'));
    }
