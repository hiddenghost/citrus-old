...
; (function ($, window, document, undefined) {
    ...
    var cityObj = function (el, option) {
		var getRealPath = function (){
			var curWwwPath = window.document.location.href;		//获取当前网址，如： http://localhost:8083/city/index.html
			var pathName = window.document.location.pathname;		//获取主机地址之后的目录，如： city/index.html
			var pos = curWwwPath.indexOf(pathName);
			var localhostPaht = curWwwPath.substring(0,pos);	//获取主机地址，如： http://localhost:8083
			var projectName = pathName.substring(0,pathName.substr(1).indexOf('/')+1);		//获取带'/'的项目名，如：/city
			var realPath = localhostPaht+projectName;			//得到了 http://localhost:8083/city
			return realPath;
		};
	
		// var domainUrl = 'https://passer-by.com/data_location/';		//采用网上接口数据
		var domainUrl = getRealPath() + 'data_location/';				//本地接口数据
		
        //默认参数
        this.defaultOptions = {
            // dataUrl: 'https://passer-by.com/data_location/list.json',    //数据库地址
            dataUrl: domainUrl + 'list.json',    //数据库地址
            ...
            townData: function (callback) {
                if (currentArea && currentArea.code) {
                    ...
                    $.ajax({
                        ...
                        url: domainUrl + 'town/' + currentArea.code + '.json',
                        dataType: 'json',
						timeout: 2000,
